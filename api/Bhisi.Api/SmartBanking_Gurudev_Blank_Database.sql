...
-- Target Database Name: SmartBanking_Gurudev
-- Generated Date: 2026-08-06
-- Includes: All 100 Tables, Primary Keys, Foreign Keys, Indexes & Latest Schema Updates
-- ============================================================================

IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = N'SmartBanking_Gurudev')
BEGIN
    CREATE DATABASE [SmartBanking_Gurudev];
END;
GO

USE [SmartBanking_Gurudev];
GO
...
-- ============================================================================
-- ESSENTIAL SEED DATA FOR BLANK PRODUCTION DATABASE
-- Includes Branch, Financial Years, Sanstha Details, User & Roles
-- ============================================================================

USE [SmartBanking_Gurudev];
GO

-- 1. Default Branch Data (मुख्य शाखा)
IF NOT EXISTS (SELECT * FROM [BranchMasters] WHERE [BranchID] = 1)
BEGIN
    SET IDENTITY_INSERT [BranchMasters] ON;
    INSERT INTO [BranchMasters] ([BranchID], [BranchCode], [BranchName], [Address], [City], [District], [State], [Pincode], [MobileNo], [Email], [Status], [CreatedDate])
    VALUES (1, N'BR001', N'मुख्य शाखा (Main Branch)', N'मध्यवर्ती कार्यालय', N'कोल्हापूर', N'कोल्हापूर', N'महाराष्ट्र', N'416001', N'9876543210', N'admin@smartbanking.in', 1, GETDATE());
    SET IDENTITY_INSERT [BranchMasters] OFF;
END;
GO

IF NOT EXISTS (SELECT * FROM [Branches] WHERE [BranchID] = 1)
BEGIN
    SET IDENTITY_INSERT [Branches] ON;
    INSERT INTO [Branches] ([BranchID], [BranchCode], [BranchName], [Address], [IFSCCode], [IsActive])
    VALUES (1, N'BR001', N'मुख्य शाखा', N'मध्यवर्ती कार्यालय', N'SBIN0000001', 1);
    SET IDENTITY_INSERT [Branches] OFF;
END;
GO

-- 2. Default Sanstha Details
IF NOT EXISTS (SELECT * FROM [SansthaDetails])
BEGIN
    INSERT INTO [SansthaDetails] ([SansthaName], [RegNo], [RegDate], [Address], [Phone], [Email], [AutoPostVoucherLimit])
    VALUES (N'श्री गुरुदेव कर्मचारी पतसंस्था मर्यादित', N'KLP/COP/12345', GETDATE(), N'कोल्हापूर', N'0231-2654321', N'info@smartbanking.in', 50000.00);
END;
GO

-- 3. Default Financial Years Data (2024-2025, 2025-2026, 2026-2027 Active)
IF NOT EXISTS (SELECT * FROM [FinancialYears] WHERE [YearCode] = N'2024-2025')
BEGIN
    INSERT INTO [FinancialYears] ([YearCode], [StartDate], [EndDate], [IsActive], [IsClosed])
    VALUES (N'2024-2025', '2024-04-01', '2025-03-31', 0, 1);
END;
GO

IF NOT EXISTS (SELECT * FROM [FinancialYears] WHERE [YearCode] = N'2025-2026')
BEGIN
    INSERT INTO [FinancialYears] ([YearCode], [StartDate], [EndDate], [IsActive], [IsClosed])
    VALUES (N'2025-2026', '2025-04-01', '2026-03-31', 0, 1);
END;
GO

IF NOT EXISTS (SELECT * FROM [FinancialYears] WHERE [YearCode] = N'2026-2027')
BEGIN
    INSERT INTO [FinancialYears] ([YearCode], [StartDate], [EndDate], [IsActive], [IsClosed])
    VALUES (N'2026-2027', '2026-04-01', '2027-03-31', 1, 0);
END;
GO

-- 4. Default Roles Data
IF NOT EXISTS (SELECT * FROM [Roles] WHERE [RoleName] = N'Admin')
BEGIN
    INSERT INTO [Roles] ([RoleName], [Description], [IsActive]) VALUES (N'Admin', N'System Administrator', 1);
    INSERT INTO [Roles] ([RoleName], [Description], [IsActive]) VALUES (N'Manager', N'Branch Manager', 1);
    INSERT INTO [Roles] ([RoleName], [Description], [IsActive]) VALUES (N'Accountant', N'Accountant', 1);
    INSERT INTO [Roles] ([RoleName], [Description], [IsActive]) VALUES (N'Cashier', N'Cashier', 1);
END;
GO

-- 5. Default Admin User Data
IF NOT EXISTS (SELECT * FROM [Users] WHERE [Username] = N'admin')
BEGIN
    INSERT INTO [Users] ([BranchID], [Username], [PasswordHash], [FullName], [Email], [MobileNo], [RoleID], [IsActive], [CreatedOn])
    VALUES (1, N'admin', N'.g3.k.E4398f8uW7x4.7j7a7a7a7a7a7a7a7a7a7a7a7a7a7a7', N'System Administrator', N'admin@smartbanking.in', N'9876543210', 1, 1, GETDATE());
END;
GO