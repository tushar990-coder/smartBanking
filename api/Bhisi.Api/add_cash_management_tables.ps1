...
$sqlScript = Get-Content -Path "d:\Bhisi Software\api\Bhisi.Api\create_cash_management_tables.sql" -Raw
...
foreach ($db in $databases) {
    ...
    try {
        ...
        $cmd.ExecuteNonQuery()
        Write-Host "SUCCESS: Cash Management tables created/verified on '$db' database!" -ForegroundColor Green
        $conn.Close()
    } catch {
