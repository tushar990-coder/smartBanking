...
export default function BalanceSheet() {
  const { user } = useAuth();
  const [fromDate, setFromDate] = useState(user?.businessDate || new Date().toISOString().split('T')[0]);
  const [toDate, setToDate] = useState(user?.businessDate || new Date().toISOString().split('T')[0]);
  ...
  const handleExportExcel = () => {
    ...
    const flattenList = (nodes: ReportNode[]): { name: string; isGroup: boolean; previousYearAmount: number; amount: number }[] => {
      ...
      for (const group of nodes) {
        ...
        result.push({
          ...
          isGroup: true,
          previousYearAmount: group.previousYearAmount || 0,
        ...
        if (group.children && group.children.length > 0) {
          for (const child of group.children) {
            ...
            result.push({
              ...
              isGroup: false,
              previousYearAmount: child.previousYearAmount || 0,
    ...
    const rows: any[] = [];
    rows.push(['ताळेबंद पत्रक (Balance Sheet)']);
    rows.push([`कालावधी: ${formatDisplayDate(fromDate)} ते ${formatDisplayDate(toDate)}`]);
    rows.push([]);
    rows.push(['देयता बाजू / भांडवल व देणी (Liabilities)', 'मागील वर्ष (₹)', 'चालू वर्ष (₹)', '', 'जिंदगी बाजू / मालमत्ता व येणी (Assets)', 'मागील वर्ष (₹)', 'चालू वर्ष (₹)']);
  ...
  const isTallied = reportData ? Math.abs(reportData.totalLiabilities - reportData.totalAssets) < 0.01 : false;
  const difference = reportData ? Math.abs(reportData.totalLiabilities - reportData.totalAssets) : 0;
        ...
        <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <h1 className="text-xs font-bold text-gray-900 tracking-tight flex items-center gap-1">
              <span>ताळेबंद पत्रक</span>
              <span className="text-[10px] font-semibold text-primary font-mono hidden sm:inline">(Balance Sheet)</span>
            </h1>
          </div>
      ...
      {reportData ? (
        ...
        >
          <div>
            ...
            {/* T-Format Balance Sheet Grid */}
            <div className="overflow-x-auto mt-2">
              <div className="flex w-full border border-gray-900">
                
                {/* Liabilities Side (Left / देयता बाजू) */}
                <div className="w-1/2 border-r border-gray-900 flex flex-col">
                  <table className="w-full text-[11px] border-collapse">
                    <thead>
                      <tr className="border-b border-gray-900 bg-gray-100/90 text-gray-900 font-bold">
                        ...
                        <th className="py-1.5 px-1 font-extrabold text-center border-r border-gray-900 w-[52%] text-xs">
                          देयता बाजू / भांडवल व देणी (Liabilities)
                        </th>
                    ...
                    <tbody className="flex-1 divide-y divide-gray-200">
                      {reportData.liabilities.map((node: ReportNode, i: number) => (
                  ...
                  <table className="w-full text-[11px] border-collapse">
                    <thead>
                      <tr className="border-b border-gray-900 bg-gray-100/90 text-gray-900 font-bold">
                        ...
                        <th className="py-1.5 px-1 font-extrabold text-center border-r border-gray-900 w-[52%] text-xs">
                          जिंदगी बाजू / मालमत्ता व येणी (Assets)
                        </th>
                        <th className="py-1.5 px-1 font-bold text-center w-[24%] text-[10px]">
                          चालू वर्ष<br/>{reportData.currentYearLabel || (toDate ? formatDisplayDate(toDate) : '३१/०३/२०२६')}
                        </th>
                    ...
                    <tbody className="flex-1 divide-y divide-gray-200">
                      {reportData.assets.map((node: ReportNode, i: number) => (
              ...
              <div className="flex w-full border-b-2 border-l border-r border-gray-900 font-bold text-gray-950 bg-gray-200/90 text-[11px]">
                <div className="w-1/2 flex border-r border-gray-900">
                  <div className="w-[24%] py-1.5 px-1 text-right border-r border-gray-900 font-mono">
                    {reportData.totalPreviousYearLiabilities ? fmtCurrency(reportData.totalPreviousYearLiabilities) : '-'}
                  </div>
                  <div className="w-[52%] py-1.5 px-1 text-center border-r border-gray-900 font-bold">एकूण देयता (Total Liabilities):</div>
                  <div className="w-[24%] py-1.5 px-1 text-right font-mono font-black text-gray-950">
                    ₹ {fmtCurrency(reportData.totalLiabilities)}
                  </div>
                </div>
                <div className="w-1/2 flex">
                  <div className="w-[24%] py-1.5 px-1 text-right border-r border-gray-900 font-mono">
                    {reportData.totalPreviousYearAssets ? fmtCurrency(reportData.totalPreviousYearAssets) : '-'}
                  </div>
                  <div className="w-[52%] py-1.5 px-1 text-center border-r border-gray-900 font-bold">एकूण मालमत्ता (Total Assets):</div>
                  <div className="w-[24%] py-1.5 px-1 text-right font-mono font-black text-gray-950">
                    ₹ {fmtCurrency(reportData.totalAssets)}
                  </div>
                </div>
              </div>

              {/* Tallied Status Badge */}
              <div className="flex justify-between items-center px-3 py-1.5 border border-gray-900 mt-2 bg-gray-50 text-[11px]">
                <span className="text-gray-700 font-medium">SmartBanking CBS ताळेबंद सत्यापन प्रणाली</span>
                <span className={`font-bold flex items-center gap-1 ${isTallied ? 'text-emerald-800' : 'text-red-600'}`}>
                  {isTallied ? (
                    <>
                      <CheckCircle2 size={13} className="text-emerald-700 inline" />
                      <span>✓ ताळेबंद पत्रक जुळले आहे (Balance Sheet Tallied)</span>
                    </>
                  ) : (
                    <>
                      <AlertTriangle size={13} className="text-red-600 inline" />
                      <span>⚠ फरक (Difference): ₹ {fmtCurrency(difference)}</span>
                    </>
                  )}
                </span>
              </div>
