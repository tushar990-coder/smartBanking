...
interface LoanCollectionRegisterDto {
...
export default function LoanCollectionRegister() {
  const [fromDate, setFromDate] = useState<string>(() => new Date().toISOString().split('T')[0]);
  const [toDate, setToDate] = useState<string>(() => new Date().toISOString().split('T')[0]);
  const [paymentFilter, setPaymentFilter] = useState<string>('Ekatrit');
  const [reportMode, setReportMode] = useState<'All' | 'ByLoanType'>('All');
  const [selectedLoanType, setSelectedLoanType] = useState<string>('');
  const [loanTypes, setLoanTypes] = useState<string[]>([]);

  const [data, setData] = useState<LoanCollectionRegisterDto[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get('/api/Reports/LoanCollectionRegister', { params });
      setData(res.data || []);
    } catch (err: any) {
      console.error('Failed to fetch report data', err);
      alert('कर्ज वसुली नोंदवही लोड करताना त्रुटी आली.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Filter items based on payment mode, loan type and search term
  const filteredData = data.filter(item => {
    if (paymentFilter === 'Cash' && item.paymentMode !== 'Cash') return false;
    if (paymentFilter === 'Transfer' && item.paymentMode === 'Cash') return false;
    if (reportMode === 'ByLoanType' && selectedLoanType && item.loanType !== selectedLoanType) return false;
  ...
  // Group by LoanType
  const groupedData: { [key: string]: LoanCollectionRegisterDto[] } = {};
  ...
  // Grand Totals
  const totalPrincipalSum = filteredData.reduce((sum, r) => sum + (r.principalCollected || 0), 0);
  const totalInterestSum = filteredData.reduce((sum, r) => sum + (r.interestCollected || 0), 0);
  const totalPenaltySum = filteredData.reduce((sum, r) => sum + (r.penaltyInterestCollected || 0), 0);
  const totalSurchargeSum = filteredData.reduce((sum, r) => sum + (r.surchargeCollected || 0), 0);
  const totalReceivedSum = filteredData.reduce((sum, r) => sum + (r.totalAmountReceived || 0), 0);
  ...
  const handleExportExcel = () => {
    ...
    filteredData.forEach((r) => {
      excelRows.push({
        ...
        'मुद्दल (₹)': r.principalCollected || 0,
        'व्याज (₹)': r.interestCollected || 0,
        'दंड व्याज (₹)': r.penaltyInterestCollected || 0,
        'इतर/अधिभार (₹)': r.surchargeCollected || 0,
        'एकूण जमा रक्कम (₹)': r.totalAmountReceived || 0,
        'पेमेंट मोड': r.paymentMode === 'Cash' ? 'रोख (Cash)' : 'ट्रान्सफर (Transfer)'
      });
    });
    ...
    excelRows.push({
      ...
      'मुद्दल (₹)': totalPrincipalSum,
      'व्याज (₹)': totalInterestSum,
      'दंड व्याज (₹)': totalPenaltySum,
      'इतर/अधिभार (₹)': totalSurchargeSum,
      'एकूण जमा रक्कम (₹)': totalReceivedSum,
      'पेमेंट मोड': ''
    });

    const worksheet = XLSX.utils.json_to_sheet(excelRows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'कर्ज वसुली रजिस्टर');
    
    const fileName = `Karj_Vasuli_Register_${fromDate}_to_${toDate}.xlsx`;
      ...
      <div className="bg-white px-3 py-2 rounded-sm shadow-xs border border-gray-200 border-b-2 border-primary mb-3 no-print space-y-1.5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <h1 className="text-xs font-bold text-gray-900 tracking-tight flex items-center gap-1">
              <span>कर्ज वसुली रजिस्टर</span>
              <span className="text-[10px] font-semibold text-primary font-mono hidden sm:inline">(Loan Collection)</span>
            </h1>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 flex-1 justify-end sm:justify-center">
            ...
            {/* Payment Mode Filter */}
            <div className="flex items-center gap-1">
              <label className="text-[11px] font-semibold text-gray-600 whitespace-nowrap">मोड:</label>
              <select
                value={paymentFilter}
              ...
              >
                <option value="Ekatrit">एकत्रित (All)</option>
                <option value="Cash">रोख (Cash)</option>
                <option value="Transfer">वर्ग (Transfer)</option>
              </select>
            </div>

            {/* Report Mode: All / By Type */}
            <div className="flex items-center gap-1">
        ...
        <div className="pt-1.5 border-t border-gray-100 flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="relative w-64 max-w-full">
            ...
            <input
              type="text"
              placeholder="खाते क्र., नाव किंवा पावती नं. शोधा..."
              value={searchTerm}
          ...
          <div className="flex items-center gap-2 text-[11px] text-gray-600">
            <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-700 border border-gray-200">
              एकूण पावत्या: <strong className="text-primary font-bold">{filteredData.length}</strong>
            </span>
            <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-700 border border-gray-200">
              मुद्दल जमा: <strong className="text-primary font-bold">₹ {fmtCurrency(totalPrincipalSum)}</strong>
            </span>
            <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-700 border border-gray-200">
              व्याज जमा: <strong className="text-amber-800 font-bold">₹ {fmtCurrency(totalInterestSum)}</strong>
            </span>
            ...
            <table className="w-full border-collapse border border-gray-900 text-xs">
              <thead>
                <tr className="bg-gray-100/90 text-gray-900 border-b border-gray-900 text-center font-bold">
                  ...
                  <th className="border border-gray-900 py-1.5 px-1.5 w-[9%] text-center">दिनांक</th>
                  <th className="border border-gray-900 py-1.5 px-1.5 w-[9%] text-center">पावती नं.</th>
                  <th className="border border-gray-900 py-1.5 px-1.5 w-[10%] text-center">खाते नं.</th>
                  <th className="border border-gray-900 py-1.5 px-2 w-[22%] text-left">कर्जदाराचे नाव</th>
                  <th className="border border-gray-900 py-1.5 px-1.5 w-[11%] text-left">कर्ज प्रकार</th>
                  <th className="border border-gray-900 py-1.5 px-2 w-[11%] text-right">मुद्दल जमा</th>
                  <th className="border border-gray-900 py-1.5 px-1.5 w-[10%] text-right">व्याज जमा</th>
                  <th className="border border-gray-900 py-1.5 px-2 w-[12%] text-right font-extrabold">एकूण जमा रक्कम</th>
                  <th className="border border-gray-900 py-1.5 px-1.5 w-[8%] text-center">Payment Mode</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  <tr>
                    <td colSpan={10} className="text-center py-8 text-gray-500 font-semibold border border-gray-900">
                  ...
                  <tr>
                    <td colSpan={10} className="text-center py-8 text-gray-500 font-semibold border border-gray-900">
                      निवडलेल्या कालावधीत कोणतीही कर्ज वसुली नोंद आढळली नाही.
                    </td>
                  </tr>
                ) : (
                  Object.keys(groupedData).map((typeStr) => {
                    const rows = groupedData[typeStr];
                    const subPrincipal = rows.reduce((sum, r) => sum + (r.principalCollected || 0), 0);
                    const subInterest = rows.reduce((sum, r) => sum + (r.interestCollected || 0), 0);
                    const subTotal = rows.reduce((sum, r) => sum + (r.totalAmountReceived || 0), 0);
                      ...
                      <React.Fragment key={typeStr}>
                        ...
                        {Object.keys(groupedData).length > 1 && (
                          <tr className="bg-gray-50 font-bold text-gray-950 border-t border-b border-gray-900">
                            <td colSpan={10} className="py-1 px-2 text-left text-primary font-bold">
                              › कर्ज प्रकार: {typeStr} ({rows.length} पावत्या)
                            </td>
                          </tr>
                        )}
                        ...
                        {rows.map((row, idx) => (
                          <tr key={row.loanCollectionID || idx} className="hover:bg-slate-50 text-gray-900 text-[11px]">
                            ...
                            <td className="border border-gray-900 py-1 px-1.5 text-right font-mono text-gray-900">
                              {fmtCurrency(row.interestCollected + (row.penaltyInterestCollected || 0) + (row.surchargeCollected || 0))}
                            </td>
                            ...
                            <td className="border border-gray-900 py-1 px-1.5 text-center font-mono text-[10.5px]">
                              <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${row.paymentMode === 'Cash' ? 'bg-emerald-50 text-emerald-800' : 'bg-blue-50 text-blue-800'}`}>
                                {row.paymentMode === 'Cash' ? 'रोख' : 'वर्ग'}
                              </span>
                            </td>
                          </tr>
                        ))}
                        ...
                        {Object.keys(groupedData).length > 1 && (
                          <tr className="bg-gray-50/80 font-bold text-gray-900 border-t border-b border-gray-800 text-[11px]">
                            <td colSpan={6} className="border border-gray-900 py-1 px-2 text-right italic">
                              उप-एकूण ({typeStr}):
                            </td>
                            <td className="border border-gray-900 py-1 px-2 text-right font-mono font-bold">
                              {fmtCurrency(subPrincipal)}
                            </td>
                            <td className="border border-gray-900 py-1 px-1.5 text-right font-mono font-bold">
                              {fmtCurrency(subInterest)}
                            </td>
                            <td className="border border-gray-900 py-1 px-2 text-right font-mono font-bold text-emerald-800">
                ...
                <tfoot>
                  <tr className="bg-gray-100 font-bold text-gray-950 border-t-2 border-gray-900 text-xs">
                    ...
                    <td className="border border-gray-900 py-1.5 px-1.5 text-right font-mono font-extrabold text-gray-950">
                      ₹ {fmtCurrency(totalInterestSum + totalPenaltySum + totalSurchargeSum)}
                    </td>
