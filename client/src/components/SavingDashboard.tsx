...
export default function SavingDashboard({ onNavigate }: SavingDashboardProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    // Observe data-theme changes on documentElement
  ...
  // 5 Circular Glossy 3D Action Buttons for Savings Department
  const savingActionButtons = [
    {
      id: 'saving-acc',
      titleMr: 'नवीन बचत खाते',
      titleEn: 'ACCOUNT OPENING',
      image: '/action-icons/saving-acc-open.jpg',
      icon: Wallet,
      tab: 'saving-account',
      params: {},
    ...
    {
      id: 'saving-tx',
      titleMr: 'बचत जमा / नावे',
      titleEn: 'DEPOSIT / WITHDRAWAL',
      image: '/action-icons/saving-deposit-tx.jpg',
      icon: Coins,
      tab: 'saving-transaction',
      params: {},
      gradient: themeCfg.gradients[1]
    },
    {
      id: 'saving-post',
      titleMr: 'बचत व्याज आकारणी',
      titleEn: 'INTEREST POSTING',
      image: '/action-icons/saving-interest-calc.jpg',
      icon: Scale,
      tab: 'saving-posting',
      params: {},
    ...
    {
      id: 'saving-pass',
      titleMr: 'पासबूक प्रिंटिंग',
      titleEn: 'PASSBOOK PRINTING',
      image: '/action-icons/saving-passbook-print.jpg',
      icon: Layers,
      tab: 'saving-passbook',
      params: {},
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-[10px] font-extrabold ${themeCfg.badgeBg} px-3 py-0.5 rounded-full shadow-2xs border`}>
                👛 बचत ठेवी विभाग (Savings Module Hub)
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-sm md:text-base font-black tracking-tight text-white drop-shadow-md flex items-center gap-2">
                    <span>बचत ठेवी विभाग मुख्य हब</span>
                  </h1>
                  <p className={`text-[11px] ${themeCfg.bannerSubText} font-medium mt-0.5 drop-shadow-xs`}>
                    नवीन बचत खाते नोंदणी, जमा/नावे व्यवहार, पासबूक प्रिंटिंग आणि व्याज आकारणी
                  </p>
                </div>
              </div>
            </div>
                      ...
                      <div className="w-full h-full rounded-full bg-white/15 backdrop-blur-2xs border border-white/40 flex items-center justify-center p-1.5 shadow-inner">
                        {btn.image ? (
                          <img
                            src={btn.image}
                            alt={btn.titleMr}
                            className="w-full h-full object-contain drop-shadow-md group-hover:scale-110 transition-transform duration-300 rounded-full"
                          />
                        ) : (
