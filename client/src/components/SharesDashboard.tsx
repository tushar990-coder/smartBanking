...
export default function SharesDashboard({ onNavigate }: SharesDashboardProps) {
  const { user } = useAuth();
  ...
  // 5 Circular Glossy 3D Action Buttons for Shares / Member Module
  const sharesActionButtons = [
    {
      id: 'member-master',
      titleMr: 'सभासद नोंदणी',
    ...
    {
      id: 'share-withdrawal',
      titleMr: 'भाग भांडवल परतावा',
      titleEn: 'SHARE WITHDRAWAL',
      icon: Receipt,
      tab: 'share-withdrawal',
      params: {},
      gradient: themeCfg.gradients[3]
    },
    {
      id: 'member-closure',
      titleMr: 'सभासद खाते बंद',
      titleEn: 'MEMBER CLOSURE',
      icon: Layers,
      tab: 'member-closure',
      params: {},
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-[10px] font-extrabold ${themeCfg.badgeBg} px-3 py-0.5 rounded-full shadow-2xs border`}>
                👥 सभासद व शेअर्स विभाग (Shares & Member Hub)
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-sm md:text-base font-black tracking-tight text-white drop-shadow-md flex items-center gap-2">
                    <span>सभासद व शेअर्स विभाग मुख्य हब</span>
                  </h1>
                  <p className={`text-[11px] ${themeCfg.bannerSubText} font-medium mt-0.5 drop-shadow-xs`}>
                    नवीन सभासद नोंदणी, भाग भांडवल खरेदी/वाटप, हस्तांतरण आणि परतावा
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className={`bg-white/95 backdrop-blur-md border ${themeCfg.cardBorder} rounded-2xl p-4 md:p-5 shadow-md space-y-3`}>
            <div className="text-center pb-2 border-b border-slate-100">
              <span className={`text-xs font-black ${themeCfg.textDark} uppercase tracking-widest flex items-center justify-center gap-1.5`}>
                <Sparkles size={14} className={themeCfg.textSub} /> सभासद व शेअर्स विभाग मुख्य हब (SHARES CORE MODULES)
              </span>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 sm:gap-6 justify-items-center pt-2 max-w-4xl mx-auto">
              {sharesActionButtons.map((btn) => {
