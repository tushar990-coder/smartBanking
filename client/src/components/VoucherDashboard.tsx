...
export default function VoucherDashboard({ onNavigate }: VoucherDashboardProps) {
  const { user } = useAuth();
  ...
  // 5 Circular Glossy 3D Action Buttons for Voucher & Accounts Hub
  const voucherActionButtons = [
    {
      id: 'voucher-receipt',
      titleMr: 'रोख जमा पावती',
      titleEn: 'CASH RECEIPT',
      icon: Receipt,
      tab: 'voucher',
      params: { type: 'receipt' },
    ...
    {
      id: 'voucher-posting',
      titleMr: 'व्हाऊचर मंजुरी',
      titleEn: 'VOUCHER POSTING',
      icon: Scale,
      tab: 'voucher-posting',
      params: {},
    ...
    {
      id: 'voucher-batch-print',
      titleMr: 'बॅच प्रिंटिंग',
      titleEn: 'BATCH PRINT',
      icon: Printer,
      tab: 'voucher-batch-print',
      params: {},
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-[10px] font-extrabold ${themeCfg.badgeBg} px-3 py-0.5 rounded-full shadow-2xs border`}>
                🧾 व्हाऊचर्स व रोख हिशोब विभाग (Voucher Module Hub)
              </span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-sm md:text-base font-black tracking-tight text-white drop-shadow-md flex items-center gap-2">
                    <span>व्हाऊचर्स व रोख हिशोब मुख्य हब</span>
                  </h1>
                  <p className={`text-[11px] ${themeCfg.bannerSubText} font-medium mt-0.5 drop-shadow-xs`}>
                    रोख जमा, पेमेंट व्हाऊचर, जर्नल नोंदी आणि बॅच प्रिंटिंग व्यवस्थापन
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className={`bg-white/95 backdrop-blur-md border ${themeCfg.cardBorder} rounded-2xl p-4 md:p-5 shadow-md space-y-3`}>
            <div className="text-center pb-2 border-b border-slate-100">
              <span className={`text-xs font-black ${themeCfg.textDark} uppercase tracking-widest flex items-center justify-center gap-1.5`}>
                <Sparkles size={14} className={themeCfg.textSub} /> व्हाऊचर्स व रोख विभाग मुख्य हब (VOUCHERS CORE MODULES)
              </span>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 sm:gap-6 justify-items-center pt-2 max-w-4xl mx-auto">
              {voucherActionButtons.map((btn) => {
