# =========================================================================================
# SmartBanking ERP - 1-Click All-in-One VPS Patch Builder
...
$zipOutputFile = Join-Path $workspaceRoot "SmartBanking_VPS_All_In_One_Patch.zip"
...
Write-Host "   SmartBanking ERP - 1-Click All-In-One VPS Patch Package Builder" -ForegroundColor Cyan
...
# -----------------------------------------------------------------------------------------
# Step 3: Prepare Clean VPS_All_In_One_Patch Directory Structure
# -----------------------------------------------------------------------------------------
...
Write-Host "  -> Database update schema copied." -ForegroundColor White
...
# 4.1 patch_config.json
$configContent = @"
{
  "SansthaName": "Shree Jotirling Nagari Sahakari PatSanstha (Padawalwadi)",
  "TargetDatabase": "SmartBanking_JotirlingPdw",
  "SqlServerInstance": ".",
  "IISAppPoolName": "api.jotirlingpdw.hellomindspace.in",
  "BackendFolderPath": "D:\\WebApps\\Jotirlingpdw\\api.jotirlingpdw.hellomindspace.in",
...
$versionJsonObj = @{
    ...
    packageType = "All-In-One VPS Patch (Frontend + Backend + Database)"
    changelog = @(
...
# -----------------------------------------------------------------------------------------
# Step 3: Stop IIS AppPool Gracefully & Release Locks
# -----------------------------------------------------------------------------------------
...
try {
    ...
    Start-Sleep -Seconds 1
    Write-Host "  -> IIS Web Server paused for clean copy." -ForegroundColor Green
} catch {}
...
if (Test-Path $config.BackendFolderPath) {
    ...
    robocopy $backendSource $config.BackendFolderPath /E /R:3 /W:1 /XF "appsettings.Production.json" "appsettings.json" "appsettings.Development.json" /XD "logs" "uploads"
...
if (Test-Path $config.FrontendFolderPath) {
    ...
    robocopy $frontendSource $config.FrontendFolderPath /E /R:3 /W:1
...
# -----------------------------------------------------------------------------------------
# Step 5: Restart IIS AppPool & Web Server
# -----------------------------------------------------------------------------------------
Write-Host "`n[5/5] Restarting IIS & AppPool '$($config.IISAppPoolName)'..." -ForegroundColor Yellow
try {
    iisreset /start | Out-Null
    Write-Host "  -> IIS Web Server restarted successfully." -ForegroundColor Green
} catch {}
if (Test-Path $appCmd) {
    try {
        ...
        Write-Host "  -> AppPool online." -ForegroundColor Gray
    }
}
...
Write-Host "  [OK] Frontend: Latest React UI Assets Deployed" -ForegroundColor Green
...
$batContent = @"
@echo off
title SmartBanking ERP - 1-Click All-in-One VPS Auto-Updater
cd /d "%~dp0"
...
echo ================================================================
echo.
echo  This tool will safely update:
echo   [1] Database Schema (Universal and Zero Data Loss)
echo   [2] Backend C# .NET 10 API (Preserves appsettings.json)
echo   [3] Frontend React Web UI (Latest Assets)
echo.
echo  Configuration Target: patch_config.json
echo.
echo ================================================================
choice /M "Do you want to apply this patch on the VPS now"
...
[System.IO.File]::WriteAllText((Join-Path $patchFolder "apply_patch.bat"), $batContent, [System.Text.Encoding]::ASCII)

# 4.5 README_INSTRUCTIONS.txt
$readmeContent = @"
===================================================================================
SMARTBANKING ERP - 1-CLICK ALL-IN-ONE VPS PATCH INSTRUCTIONS
===================================================================================

This package contains:
1. Database Schema Update (`database/update_schema.sql`) - Zero Data Loss
2. Backend .NET 10 Web API (`backend/`) - High Performance C# Binaries
3. Frontend React Web UI (`frontend/`) - Modern Marathi & English Banking UI
4. Automatic Installer Script (`Apply_VPS_Patch.bat` & `apply_patch.ps1`)

-----------------------------------------------------------------------------------
HOW TO APPLY ON VPS (3 SIMPLE STEPS):
-----------------------------------------------------------------------------------
Step 1: Copy or Extract this entire folder on your VPS (e.g. C:\SmartBanking_Patch).
Step 2: (Optional) Open `patch_config.json` and verify your settings:
        - TargetDatabase: Your database name (e.g. SmartBanking_JotirlingPdw)
        - BackendFolderPath: (e.g. C:\inetpub\wwwroot\api_publish)
        - FrontendFolderPath: (e.g. C:\inetpub\wwwroot\client_dist)
        - IISAppPoolName: (e.g. SmartBanking_API)
Step 3: Right-click `Apply_VPS_Patch.bat` -> "Run as administrator" (किंवा डबल क्लिक करा).

That's it! The script will automatically:
- Take a timestamped safety database backup (.bak)
- Apply all database updates non-destructively
- Stop IIS AppPool
- Copy backend & frontend files (preserving your connection strings)
- Restart IIS AppPool
===================================================================================
"@
[System.IO.File]::WriteAllText((Join-Path $patchFolder "README_INSTRUCTIONS.txt"), $readmeContent, $utf8NoBom)

# -----------------------------------------------------------------------------------------
# Step 5: Compress into Single .zip Archive
# -----------------------------------------------------------------------------------------
Write-Host "`n[5/5] Compressing All-in-One VPS Patch into ZIP archive..." -ForegroundColor Cyan
...
if (Test-Path $desktopPath) {
    $desktopZip = Join-Path $desktopPath "SmartBanking_VPS_All_In_One_Patch.zip"
    Copy-Item -Path $zipOutputFile -Destination $desktopZip -Force
    Write-Host "  -> Copied to Desktop: $desktopZip" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "==================================================================" -ForegroundColor Green
Write-Host "     ALL-IN-ONE VPS PATCH CREATED SUCCESSFULLY!                   " -ForegroundColor Green
Write-Host "==================================================================" -ForegroundColor Green
Write-Host "  -> ZIP File : $zipOutputFile ($zipSizeMb MB)" -ForegroundColor Green
Write-Host "  -> Folder   : $patchFolder" -ForegroundColor Green
if (Test-Path $desktopPath) {
    Write-Host "  -> Desktop  : $(Join-Path $desktopPath 'SmartBanking_VPS_All_In_One_Patch.zip')" -ForegroundColor Green
}
Write-Host "  -> Modules  : Frontend + Backend (.NET 10) + Database (SQL)" -ForegroundColor Green
Write-Host "==================================================================" -ForegroundColor Green
Write-Host ""
