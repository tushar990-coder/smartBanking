# =========================================================================================
# SmartBanking ERP - 1-Click Dedicated VPS Patch Builder for Template Application
# Automates: Frontend Build (Vite) + Backend Publish (.NET 10 API) + Database SQL Patch + Packaging
# Targets ONLY: Template Application (SmartBanking_Template / D:\WebApps\_template)
# =========================================================================================
...
$patchFolder = Join-Path $workspaceRoot "SmartBanking_Template_Patch"
$zipOutputFile = Join-Path $workspaceRoot "SmartBanking_Template_Patch.zip"
...
$desktopDest = Join-Path $desktop "SmartBanking_Template_Patch.zip"

Write-Host "==================================================================" -ForegroundColor Cyan
Write-Host "   SmartBanking ERP - Dedicated Template Patch Builder            " -ForegroundColor Cyan
Write-Host "   Target Version: v$version ($buildDate)                         " -ForegroundColor Yellow
Write-Host "   Target Site   : Template Application (_template)               " -ForegroundColor Yellow
Write-Host "   Target DB     : SmartBanking_Template                          " -ForegroundColor Yellow
Write-Host "==================================================================" -ForegroundColor Cyan
Write-Host ""
...
$backendTempPublish = Join-Path $workspaceRoot "scratch\api_publish_temp_template"
...
# -----------------------------------------------------------------------------------------
# Step 3: Prepare Clean SmartBanking_Template_Patch Directory Structure
# -----------------------------------------------------------------------------------------
...
# 4.1 patch_config.json strictly for Template Application
$configContent = @"
{
  "SansthaName": "Template",
  "TargetDatabase": "SmartBanking_Template",
  "SqlServerInstance": ".",
  "SqlUser": "sa",
  "SqlPassword": "Mindspace2026",
  "IISAppPoolName": "",
  "BackendFolderPath": "D:\\WebApps\\_template\\backend",
  "FrontendFolderPath": "D:\\WebApps\\_template\\frontend",
  "TakeDbBackup": true,
...
$applyPs1Content = @'
# =========================================================================================
# SmartBanking Core ERP - 1-Click VPS Patch Installer (Template Application)
...
Clear-Host
Write-Host "==================================================================" -ForegroundColor Cyan
Write-Host "  SmartBanking ERP - 1-Click VPS Template Patch Installer        " -ForegroundColor Cyan
...
# Step 3: Apply Database Schema Sync
Write-Host "`n[3/5] Synchronizing Database Schema (Zero Data Loss)..." -ForegroundColor Cyan
...
if (Test-Path $sqlFile) {
    try {
        ...
        Write-Host "  -> ADO.NET Notice, applying via sqlcmd..." -ForegroundColor DarkYellow
        cmd /c "sqlcmd -S `"$instance`" -d `"$targetDb`" -U `"$sqlUser`" -P `"$sqlPassword`" -i `"$sqlFile`" -b"
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  -> Database schema synchronized via sqlcmd!" -ForegroundColor Green
        } else {
            Write-Host "  -> [WARNING] Schema sync finished with warnings (non-fatal)." -ForegroundColor DarkYellow
        }
    }
} else {
    Write-Host "  -> No database\update_schema.sql found (Skipped)." -ForegroundColor Gray
}
...
Write-Host "  SUCCESS: Template Application Patch applied successfully!      " -ForegroundColor Green
Write-Host "==================================================================" -ForegroundColor Green
Write-Host ""
'@
[System.IO.File]::WriteAllText((Join-Path $patchFolder "apply_patch.ps1"), $applyPs1Content, $utf8WithBom)

# 4.3 Apply_Template_Patch.bat
$batContent = @"
@echo off
chcp 65001 >nul
...
cls
echo ==================================================================
echo   SmartBanking ERP - 1-Click Update: Template Application
...
pause
"@
[System.IO.File]::WriteAllText((Join-Path $patchFolder "Apply_Template_Patch.bat"), $batContent, [System.Text.Encoding]::ASCII)

# -----------------------------------------------------------------------------------------
# Step 5: Compress to ZIP and Export to Desktop
# -----------------------------------------------------------------------------------------
Write-Host "`n[5/5] Packaging into SmartBanking_Template_Patch.zip..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "==================================================================" -ForegroundColor Green
Write-Host "  SUCCESS: Template Patch packaged successfully!                  " -ForegroundColor Green
